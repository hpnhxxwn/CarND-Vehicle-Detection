...
from HMWindows import HMWindows
...
hm_windows = HMWindows(num_frames)
svc = LinearSVC()
...
def annotate_image(image):
	...
	"""
	global hm_windows, svc, X_scaler
	...
	new_hm_windows = search_windows(image, windows, svc, X_scaler, color_space=color_space,
	...
	# DEBUG
	window_img = draw_boxes(draw_image, new_hm_windows, color=(0, 0, 255), thick=6)
	#return window_img

	# Add new hot windows to HotWindows queue
	hm_windows.add_windows(new_hm_windows)
	all_hm_windows = hm_windows.get_windows()

	# Calculate and draw heat map
	heatmap = np.zeros((720, 1280))  # NOTE: Image dimensions hard-coded
	heatmap = add_heat(heatmap, all_hm_windows)
